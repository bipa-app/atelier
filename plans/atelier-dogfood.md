...
> **Why**: battle-test before any publish decision. The product's own PRD names three ad-hoc substrates Bipa built; the test is whether atelier replaces them for the agents on this machine.
> **Linear project**: https://linear.app/bipa/project/atelier-6df1165b35cf
...
2. **Wiring** — MCP registration per harness: Claude Code (`.mcp.json`), codex (`mcpServers` config), OMP (mcp servers), bip (daemon MCP config). One kit documents all four, copy-paste exact.
...
`docs/integrations.md`: exact MCP registration for Claude Code, codex, OMP, and bip; the AGENTS.md paragraph a dogfood project pastes; the skill text agents load. Ships with a live proof: an agent session over MCP edits an adopted real repo and lands.

### D3: dogfood bip sessions on atelier workspaces
bip's daemon gives sessions worktrees today; the dogfood is a bip session whose working copy is an atelier session working copy, journal and gate included. Lives in bipa-app/bip; this card tracks the atelier-side needs it surfaces.

### D4: the feedback loop
Dogfood pain lands as Linear cards the day it is felt, tagged to this project. Known candidates already: sync-back for folder sources, bookmark motion on landing (landed work should advance the source's branch), `status` read model, session working copies outside `.atelier` for build-tool friendliness.
