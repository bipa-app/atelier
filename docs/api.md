...
The landing lease, request state, and journal writes go through one coordination port. Local implementation: SQLite (WAL) — atomic claim in a transaction, TTL column, correct across CLI + server processes. Every claim opens a numbered tenancy (the point's epoch bumps), and every leased line move — a landing, an undo, a fold — runs a fence before anything publishes: the fence renews the tenancy its epoch names and refuses by name (`LeaseSuperseded`) once a newer claim superseded it; expiry alone renews (ADR-0014). Hosted serving keeps the same store on the owning node and replicates it to the workspace's bucket; single-writer per workspace is the ownership record's fencing epoch, not a second coordination system (ADR-0013). No bespoke coordination, ever. Leases stay internal — `approve`/`land` acquire them; `status()` exposes them read-only.

### Errors (typed; the contract includes failure)

`NotAWorkspace` · `WorkspaceExists` · `NestedWorkspace` · `GitRepositoryExists` · `GitFoldConflicted { branch }` · `LinkedWorktreeUnsupported { folder, repository }` · `AlreadyAttached` · `LfsSourceUnsupported` (attach-time, ADR-0002) · `NoActorConfigured` · `SessionNotFound` · `SessionClosed` · `RequestNotFound` · `ApprovalNotAuthorized` · `SelfApprovalForbidden` (profile) · `ApprovalsDismissed { new_snapshot }` · `LeaseHeld { holder, expires }` · `LeaseSuperseded { point }` · `LandParkedOnConflict { request, conflicts }` · `FileTooLarge { path, limit }` · `PackageFailed { package, fell_back_to }` · `WindowTooLarge { max }` · `AddressNotFound { available }`
...
POST /v1/sources/{mount}/sync         mirror the line to its origin; body may carry force (ADR-0010/0012)
POST /v1/sources/{mount}/pull         fold bucket changes into the line (ADR-0012)
```
...
### Attach
- Folder already a git repo → LocalGit source: preserve history, colocate. Already jj → adopt. Inside another workspace → `NestedWorkspace`. LFS git source → loud `LfsSourceUnsupported` (ADR-0002). Re-attach → `AlreadyAttached`. A `.git` FILE (linked worktree, submodule checkout) → refuse before copying: `LinkedWorktreeUnsupported` when its pointer identifies the owning repository, otherwise a config error. Both name the standalone-clone move: `git clone --no-local --single-branch -- <worktree> <new-source>`, then `atelier attach <new-source> --mount <name>`. Cloning takes the worktree's committed HEAD, never uncommitted edits; `--no-local` avoids a shared object-store dependency. Attaching the owning repository instead could select a different branch.
...
- **Publishing a Git source** → landing writes the commit and `refs/heads/<branch>` inside the workspace mount, using the branch saved at attachment or `atelier` for detached adoption. The original clone stays unchanged. Publish with `git -C <workspace>/<mount> push <remote> refs/heads/<branch>:refs/heads/<branch>`; the explicit refspec works with the mount's detached HEAD. `atelier sync <mount>` refuses with the mount's absolute path, exact ref, and publish command. Choose a configured remote or a URL; a clone made from a linked worktree initially points `origin` at that worktree.
